 ...
 * norm (data/cohort_norms/<assignment>.yaml, regenerated by
 * frontend/scripts/compute-cohort-norms.py from the emailed ground-truth
 * files) and produces advisory flags for the teacher:
 *
 *   Signal A — global count flag (coarse): total checked count exceeds
 *     max(cohort_median * 1.5, cohort_median + 10). Catches only
 *     count outliers (e.g. 2026SS_99-style +34 extras).
 ...
 *     appear in the cohort's "typical review" set (items checked in ≥ 50%
 *     of the emailed ground truths), the count looks normal but the
 *     selection differs — show a note on the review page.
 *
 ...
 * the norms YAML is keyed by the professor's form names (camelCase, as in
 * the emailed ground-truth files). Categories without a norm entry (e.g.
 * `genai`) are simply never flagged.
 */
